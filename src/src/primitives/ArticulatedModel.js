        ...
        function getComponentTree(children) {
            ...
            children.forEach((child) => {
                if (child instanceof Rig || child instanceof Mesh) {
                    ...
                    components[child.getName()].children = getComponentTree(child.getChildren());
                    components[child.getName()].component = child;
                }
            });
            return components;
        }
        return getComponentTree(this.getChildren());
    }
    ...
    static getRigs(obj) {
        ...
        obj.getChildren().forEach((child) => {
            ...
            if (child instanceof Rig) {
                if (child.getId() in rigs) throw new Error(`Rig with id ${child.getId()} already exists in model.`);
        ...
        Object.keys(modelDefinition).forEach((objName) => {
            ...
            if (objName[0] == "R") {
                ...
                model._materials[objName] = [
                    new PhongMaterial(),
                    new BasicMaterial("Basic" + objName[0], new Color(0, 0, 1, 1)),
                ];
                child = new Mesh(
            ...
            if (md.children)
                this.fromModel(md.children, model, child);
        });
        return model;
    }
};